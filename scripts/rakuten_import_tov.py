"""
Rakuten → Supabase Import: TOV Furniture
-----------------------------------------
Same pipeline as rakuten_import_ashley.py — just different MID and retailer slug.
Column positions are identical; availability code lives in col[22] for TOV.

Usage:
  python3 scripts/rakuten_import_tov.py
"""
...
def parse_row(values):
    ...
    # TOV: col[21] is empty; availability code is in col[22]
    ...
    return {
        ...
        "currency":      values[COL["currency"]].strip() or "USD",
...
def upsert_batch(url, headers, batch):
    try:
        ...
        print("\n❌  'requests' library not found. Run: pip3 install requests")
        sys.exit(1)
...
def main():
    ...
    env = load_env()
    FTP_USER     = env.get("RAKUTEN_FTP_USER")
    FTP_PASS     = env.get("RAKUTEN_FTP_PASS")
    ...
    if not local_path:
        ...
        for fname in FEED_FILENAMES:
            if fname in available:
                ...
                break
        if not target:
            print(f"❌  No TOV feed file found. Available .gz files:")
            print(f"    {[f for f in available if f.endswith('.gz')]}")
            ftp.quit()
            sys.exit(1)
