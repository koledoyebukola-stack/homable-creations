...
export async function searchProducts(itemId: string): Promise<Product[]> {
  ...
  // Call the edge function to search for products - FIXED: send detected_item_id instead of item_id
  const { data, error } = await supabase.functions.invoke('app_8574c59127_search_products', {
    body: {
      detected_item_id: itemId,  // FIXED: Changed from item_id to detected_item_id
    },
  });
  ...
  console.log('[Frontend] Edge function returned:', data);
