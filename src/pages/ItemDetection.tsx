...
// Helper function to clean item name for social media URLs
function cleanItemNameForUrl(itemName: string): string {
  return itemName
    .toLowerCase()
...
export default function ItemDetection() {
  ...
  const createInstagramSearchUrl = (productTitle: string) => {
    const cleanedTitle = cleanItemNameForUrl(productTitle);
    return `https://www.instagram.com/explore/tags/${cleanedTitle}/`;
  };

  const createTikTokSearchUrl = (productTitle: string) => {
    const cleanedTitle = cleanItemNameForUrl(productTitle);
    return `https://www.tiktok.com/tag/${cleanedTitle}`;
  };
                        ...
                        {customMessage || itemProducts.length === 0 ? (
                          <Card className="bg-gradient-to-br from-gray-50 to-white border-gray-200 shadow-sm">
                            <CardContent className="p-6 md:p-8 text-center space-y-4">
                              ...
                              <p className="text-sm text-[#666666]">
                                Search for similar items on your favorite platforms
                              </p>
                              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                                ...
                                <Button
                                  onClick={() => window.open(createInstagramSearchUrl(item.item_name), '_blank')}
                                  variant="outline"
                                  className="rounded-full border-2 border-[#E4405F] text-[#E4405F] hover:bg-[#E4405F] hover:text-white"
                                  size="sm"
                                >
                                ...
                                <Button
                                  onClick={() => window.open(createTikTokSearchUrl(item.item_name), '_blank')}
                                  variant="outline"
                                  className="rounded-full border-2 border-[#000000] text-[#000000] hover:bg-[#000000] hover:text-white"
                                  size="sm"
                                >
                                  <svg className="mr-2 h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
                          ...
                          <>
                            ...
                            <div className="text-center pt-4">
                              <p className="text-sm text-[#555555] mb-3">
                                Need more options? Search on other platforms
                              </p>
                              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                                <Button
                                  onClick={() => window.open(createGoogleSearchUrl(item.item_name), '_blank')}
